...
import { quoteUsdToBrlExchangeRate } from './client.ts'

describe('quoteUsdToBrlExchangeRate function', () => {
  const date = new Date()

  const jsonFunctionMock = jest.fn(async () => ({ value: [{ cotacaoCompra: 10 }] }) as unknown)
  ...
  it('should throw an exception when the API response is not an HTTP 200', async () => {
    ...
    await expect(quoteUsdToBrlExchangeRate(date)).rejects.toThrow()
  })
  ...
  it('should resolve to a number when the API responds successfully', async () => {
    await expect(quoteUsdToBrlExchangeRate(date)).resolves.toBe(10)
  })
})
