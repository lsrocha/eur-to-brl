...
import { generateIncomeTaxReport, type ReportType } from './report/report-generator.ts'
...
type CommandExecutor = (options: ReturnType<typeof parseArgs>['values']) => Promise<void>
...
const commands: {
  ...
  csv: {
    ...
    execute: async (
      ...
      const reportType: ReportType =
        options.type && REPORT_TYPES.includes(options.type) ? options.type : DEFAULT_REPORT_TYPE
  ...
  'single-amount': {
    ...
    execute: async (options: Partial<{ amount: string; date: string }>) => {
      ...
      const convertedAmount = await convertFromEurToBrl(parsedAmount, parsedDate)
...
async function main() {
  const { values: options, positionals: args } = parseArgs({
    options: Object.assign({}, ...Object.keys(commands).map((commandName) => commands[commandName].options)),
